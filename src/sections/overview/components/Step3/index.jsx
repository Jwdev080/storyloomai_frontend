...
const App = () => {
  ...
  const response = useSelector((state) => state.products.productData.episode);

  const [title, setTitle] = useState("They are lost in a soul city");
  const [character1Info, setCharacter1Info] = useState("character1 is a small annoying robot, that talks to much and thinks it knows everything.");
  const [character2Info, setCharacter2Info] = useState("character2 is A sci fi gun slinger, whose first response is to shoot.");
  const [storyIdea, setStoryIdea] = useState("They are lost in a soul city");
